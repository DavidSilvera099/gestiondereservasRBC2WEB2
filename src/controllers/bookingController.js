...
export const getAllBookings = async (req, res) => {
    try {
        const [bookings, fields] = await db.query('SELECT * FROM bookings');
        res.json(bookings);
    } catch (error) {
...
export const getBookingById = async (req, res) => {
    try {
        const [booking, fields] = await db.query('SELECT * FROM bookings WHERE idbookings = ?', [req.params.id]);
        if (booking.length === 0) {
...
export const createBooking = async (req, res) => {
    try {
        ...
        const [result] = await db.query('INSERT INTO bookings (codigo_habitacion, nombre_cliente, telefono_cliente, fecha_reservación, fecha_entrada, fecha_salida) VALUES (?, ?, ?, ?, ?, ?)', [codigo_habitacion, nombre_cliente, telefono_cliente, fecha_reservación, fecha_entrada, fecha_salida]);
...
export const updateBooking = async (req, res) => {
    try {
        ...
        const [result] = await db.query('UPDATE bookings SET nombre_cliente = ?, telefono_cliente = ?, fecha_reservación = ?, fecha_entrada = ?, fecha_salida = ? WHERE idbookings = ?', [nombre_cliente, telefono_cliente, fecha_reservación, fecha_entrada, fecha_salida, req.params.id]);
...
export const deleteBooking = async (req, res) => {
    try {
        const [result] = await db.query('DELETE FROM bookings WHERE idbookings = ?', [req.params.id]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Booking not found' });
        }
